...
from .multimodal_deformable_cross_attention import DeformableCrossAttention

from mmdet3d.models import DETECTORS

@DETECTORS.register_module()
class MFAFuser(nn.Module):
    def __init__(self, type='MFAFuser', num_sweeps=1, img_dims=80, pts_dims=128, 
        embed_dims=256, num_layers=6, num_heads=4, bev_shape=(128, 128)):
        ...
        self._pos_dim_ = self.embed_dims//2     #self.
        self._ffn_dim_ = self.embed_dims*2      #self.
        ...
        #try1 code- none type object error
        # self.bev_h = None
        # self.bev_w = None
        ...
        for (h, w) in self.bev_shape:
            ...
            # ref_2d = self.get_reference_points(h, w).to(torch.cuda.current_device())
            self.register_buffer(f'ref_2d_{h}x{w}', self.get_reference_points(h, w).to(torch.cuda.current_device()))
            # self.ref_2d_buffers[(h, w)] = ref_2d

        ffn_cfgs = dict(
            type='FFN',
            embed_dims=self.embed_dims,
            feedforward_channels=self._ffn_dim_,
            num_fcs=2,
            ffn_drop=0.1,
    ...
    def _forward_single_sweep(self, feat_img, feat_pts, feat_index):
        ...
        bs = feat_img.shape[0]
        ref_2d = getattr(self, f'ref_2d_{feat_img.shape[-2]}x{feat_img.shape[-1]}')
        ref_2d_stack = ref_2d.repeat(bs, 1, 1, self.num_modalities, 1)
        ...
        bev_mask = torch.zeros((bs, feat_img.shape[-2], feat_img.shape[-1]),
                               device=bev_queries.device).to(feat_img.dtype)
        # self.positional_encoding = self.positional_encoding.to(feat_img.device)       #SBEV
        self.positional_encoding = self.positional_encodings[feat_index].to(feat_img.device)
        bev_mask = bev_mask.to(feat_img.device)
        bev_pos = self.positional_encoding(bev_mask).to(feat_img.dtype)
        # bev_pos = self.positional_encoding(bev_mask.to(feat_img.device)).to(feat_img.dtype)
    ...
    def forward(self, feats, feat_index, times=None):

        #WRONG - DYNAMICALLY BUILDING NETWORK IN FORWARD
        # _, _, _, self.bev_h, self.bev_w = feats.shape     #init start
        # self.positional_encoding = build_positional_encoding(
        #     dict(
        #         type='LearnedPositionalEncoding',
        #         num_feats=self._pos_dim_,
        ...
        #     ),
        # )
        # self.register_buffer('ref_2d', self.get_reference_points(self.bev_h, self.bev_w).to(torch.cuda.current_device()))   #init end
