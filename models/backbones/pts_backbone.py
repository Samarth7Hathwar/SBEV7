...
from models.necks.second_fpn import SECONDFPN_Custom
...
class PtsBackbone(nn.Module):
    ...
    def __init__(self,
        ...
        mid_channels = pts_backbone['out_channels']     #[-1] DARC
        if pts_neck is not None:
            self.pts_neck = builder.build_neck(pts_neck)  #original
            # mid_channels = sum(pts_neck['out_channels'])  #original
            mid_channels = (pts_neck['out_channels'])
        else:
            self.pts_neck = None
        ...
        if self.return_context:
            ...
            self.pred_context = nn.ModuleList([
                nn.Sequential(
                nn.Conv2d(in_ch, in_ch // 2, kernel_size=3, stride=1, padding=1, padding_mode='reflect'),
                nn.BatchNorm2d(in_ch // 2),
                nn.ReLU(inplace=True),
                nn.Conv2d(in_ch // 2, out_channels, kernel_size=1, stride=1, padding=0),
                ) for in_ch in mid_channels])
        ...
        if self.return_occupancy:
            ...
            self.pred_occupancy = nn.ModuleList([
                nn.Sequential(
                nn.Conv2d(in_ch, in_ch // 2, kernel_size=3, stride=1, padding=1, padding_mode='reflect'),
                nn.BatchNorm2d(in_ch // 2),
                nn.ReLU(inplace=True),
            ...
            for pred_occ in self.pred_occupancy:
                pred_occ[-1].bias.data.fill_(bias_init_with_prob(occupancy_init))
    ...
    def _forward_single_sweep(self, pts):
        ...
        # if self.return_context:
        #     # if self.pts_neck is not None:
        #     #     x_context = self.pred_context(x[-1].float()).unsqueeze(1)
        #     # else:
        #     x_context_list = []
        #     for i in range(0,len(x)):
        #         x_context = self.pred_context[i](x[i].float()).unsqueeze(1)
        #         x_context_list.append(x_context)
        #     x_context = x_context_list
        # if self.return_occupancy:
        #     # if self.pts_neck is not None:
        #     #     x_occupancy = self.pred_occupancy(x[-1].float()).unsqueeze(1).sigmoid()
        #     # else:
        #     x_occupancy_list = []
        #     for i in range(0,len(x)):
        #         x_occupancy = self.pred_occupancy[i](x[i].float()).unsqueeze(1).sigmoid()
        #         x_occupancy_list.append(x_occupancy)
        #     x_occupancy = x_occupancy_list
    ...
    def forward(self, ptss, times=None):
        ...
        # key_context, key_occupancy = self._forward_single_sweep(ptss[:, 0, ...])  #DARC
        key_context = self._forward_single_sweep(ptss[:, 0, ...])   
        ...
        if num_sweeps == 1:
            ...
            return key_context, self.times

        #code below is of DARC for multiple radar sweeps
